...
**资源管理**
- 新节点文件存储池自动规划容量，后台手动扩容，检查宿主预留及同分区池文件承诺空间
- 多节点接入与监控，资源统计与告警，一键连通性测试，CPU/内存/磁盘超开比率，动态内存，KSM 状态展示
- 节点分组与集群，实例在线热迁移，HA 自动疏散与维护模式，节点停用与退役迁移
- 网络向导，NAT 模式一键配置 DHCP/NAT 和共享 IP，公网模式支持桥接或路由（Proxy ARP）接入，IPv6 支持 routed NIC 模式
- 容器/虚拟机全生命周期管理（创建、启停、重装、快照、批量操作），支持保留订单和资源分配的原节点实例重建
- 基于 OVN 的 VPC 私有网络，子网划分与安全组
- IP 池管理（节点绑定、单 IP 增删、状态管理），灵活的 IP 分配与回收，支持顺序/随机/按需分配和分配区间限制，IPv6 池支持按需分配（无需预生成），支持纯 IPv6 套餐（不分配 IPv4）
...
**计费与销售**
- 套餐管理，支持按月/季/年/小时等多种计费周期，CPU 限制（容器支持小数核心如 0.5 核），试建验证，支持复制套餐并按地区设置独立库存
- 流量包（叠加/重置两种模式），按套餐限制可购范围
- 完整订单流程，自动扣费与余额不足暂停，资源预检，后台首期免费开通（后续正常续费），续费价格策略（跟随套餐/锁定购买价），退款策略定制（时间窗口/流量阈值/次数限制）
- 实例转移（Push），用户间通过转移码或邮箱邀请转移实例所有权，支持收费、实名要求、管理员审批
- 库存管理，资源不足自动拦截下单
- 支付集成（支付宝/微信/Stripe/PayPal/易支付），可扩展更多渠道，支持按渠道配置手续费
- 优惠券与促销，支持限制适用订单类型和套餐
- 发票管理（申请、审核、录入、在线查看下载）
- 代理商分组，差异化返佣与分销折扣矩阵
- 用户自助申请代理，支持自动通过或管理员审核

**用户与安全**
- 邮箱/手机号注册登录，多邮箱管理，TOTP 两步验证
- 实名认证（二要素 / 人脸识别），内置阿里云、腾讯云、支付宝渠道
- 后台路径自定义，增强安全性
